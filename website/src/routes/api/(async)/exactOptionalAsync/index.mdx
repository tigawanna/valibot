...
When using `exactOptionalAsync` in a <Link href="/api/pipeAsync/">`pipeAsync`</Link>, the pipe actions only execute if a `default_` value is provided or the key is present. This applies to all pipe actions including <Link href="/api/transformAsync/">`transformAsync`</Link>, <Link href="/api/checkAsync/">`checkAsync`</Link>, and others.
